...
export function banner(): void {
  ...
  writeln();
  writeln(gradient('  ██████╗ ██████╗ ██████╗ ███╗   ██╗', 255, 215, 0, 255, 165, 0));
  writeln(gradient('  ██╔═══╝ ██╔═══╝ ██╔══██╗████╗  ██║', 255, 200, 0, 255, 140, 0));
  writeln(gradient('  ██║     ██║  ██╗██████╔╝██╔██╗ ██║', 255, 185, 0, 255, 115, 0));
  writeln(gradient('  ██║     ██║  ██║██╔══██╗██║╚██╗██║', 255, 170, 0, 255, 90, 0));
  writeln(gradient('  ██████╗ ██████╔╝██║  ██║██║ ╚████║', 255, 155, 0, 255, 65, 0));
  writeln(gradient('  ╚═════╝ ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═══╝', 255, 140, 0, 255, 40, 0));
  writeln();
  writeln(`  ${DIM}${BRIGHT_WHITE}AI Agent Intelligence Platform — One-Command Installer${RESET}`);
...
export function thinking(message: string): SpinnerHandle {
  ...
  const interval = setInterval(() => {
    ...
    clearCurrentLine();
    write(`  ${BRIGHT_MAGENTA}${dot}${RESET} ${ITALIC}${DIM}${currentMsg}${RESET} ${DIM}— ${thought}${RESET}`);
    frame++;
  }, 100);
